...
export const getUserInputRouteHandler = (req, res) => {
  const meUser = req.user;
  ...
  const sentData = {
    data: {
      ...
      attributes: {
        empid: meUser.empid,
        name:  meUser.name,
        // email: meUserInput.email,
        department: meUser.department,
        status: meUser.status,
        team: meUser.team,
        shift: meUser.shift,
        manager: meUser.manager
        // profile_image: null,
        // createdAt: meUser.createdAt,
        // updateAt: meUser.updateAt
      },
      links: {
        self: `${process.env.APP_URL_API}/userinput`
      }
    }
  }
  res.send(sentData);
...
export const getPmInputRouteHandler = (req, res) => {
  const meUser = req.user;
  ...
  const sentData = {
    data: {
      ...
      id: intId === 1 ? intId : meUser.id,
      attributes: {
        addproject: meUser.addproject,

      },
      links: {
        self: `${process.env.APP_URL_API}/pminput`
      }
    }
  }
  res.send(sentData);
...
export const patchProfileRouteHandler = async (req, res) => {
  ...
  const { empid, name, email, newPassword, confirmPassword } = req.body.data.attributes;
  ...
  if (!foundUser) {
    ...
    if (newPassword && newPassword < 7 || newPassword != confirmPassword) {
      ...
      try{
        await userModel.updateOne( { email: foundUser.email }, { $set :{ "empid": empid, "name": name, "email": email, "password": hashPassword } });
      } catch(err) {
        console.error(err);
      }
      const sentData = {
      ...
      try {
        await userModel.updateOne( { email: foundUser.email }, { $set :{ "empid": empid, "name": name, "email": email } });
      } catch(err) {
        console.error(err);
      }
      const sentData = {
